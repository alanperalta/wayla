...
if(!$do_login['error']){
    ...
    if(!$getDataResultPasajero['error']) {
        if(count($getDataResultPasajero['data']) > 0){
            ...
            $data['cp'] = (string)$getDataResultPasajero['data'][0]['CP'];
            $data['localidad'] = (string)$getDataResultPasajero['data'][0]['Z_FK_ERP_LOCALIDADES'];
            $data['tel'] = (string)$getDataResultPasajero['data'][0]['TEL1'];
            $data['email'] = (string)$getDataResultPasajero['data'][0]['EMAIL1'];
            $data['nombre_RL'] = (string)$getDataResultPasajero['data'][0]['NOMBRE_RL'];
            $data['dni_RL'] = (string)$getDataResultPasajero['data'][0]['NUM_DOC_RL'];
            $data['tel_RL'] = (string)$getDataResultPasajero['data'][0]['TEL2'];
            $data['email_RL'] = (string)$getDataResultPasajero['data'][0]['EMAIL2'];
...
for($pagina = 1; $pagina <= $paginas ; $pagina++){
    ...
    if($pagina == 1){
        ...
        $pdf->SetXY(55, 190);
        $pdf->Cell(100, 4, utf8_decode($data['nombre_RL']));
        $pdf->SetXY(107, 197);
